...
def detect_gems_v4(screen):
    ...
    for cnt in contours:
        ...
        if bg_frac >= min_bg_frac and text_pixels >= min_text_px:
            # Tighten bounds based on text/border location if background is wide
...
for r in res:
    ...
    cv2.drawMarker(vis, (x + w//2, y + h//2), (0, 0, 255), cv2.MARKER_CROSS, 8, 2)
    cv2.putText(vis, "GEM", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)

cv2.imwrite(r"C:\Users\gregg\.gemini\antigravity-ide\brain\bc444d8e-d778-41d6-a9f9-4d86757c5719\gems_v4_result.png", vis)
print("Saved gems_v4_result.png")
